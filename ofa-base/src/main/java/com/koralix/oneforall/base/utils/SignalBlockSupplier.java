...
public interface SignalBlockSupplier {
    void addComponents(ItemStack item, int signalStrength);

    default ItemStack getItemStack(Block block, int signalStrength) {
        ItemStack item = new ItemStack(block);
        addComponents(item, signalStrength);
    ...
    @Contract(pure = true)
    static @NotNull SignalBlockSupplier container(int slots) {
        return (stack, signalStrength) -> {
            if (isInvalidSignalStrength(signalStrength, 897))
                throw new IllegalArgumentException("Container signal must be 0-897");
    ...
    private static int getItemsNeeded(int slots, int signalStrength, @NotNull Item item) {
    ...
    @Contract(pure = true)
    static @NotNull SignalBlockSupplier composter() {
        return (stack, signalStrength) -> {
            ...
            stack.set(DataComponentTypes.BLOCK_STATE, BlockStateComponent.DEFAULT.with(Properties.LEVEL_8, signalStrength));
        };
    }

    @Contract(pure = true)
    static @NotNull SignalBlockSupplier commandBlock() {
        return (stack, signalStrength) -> {
    ...
    private static void setItemName(@NotNull ItemStack item, int signalStrength) {
